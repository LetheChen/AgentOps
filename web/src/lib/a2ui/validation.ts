...
import {
  ...
  analyzeAgentopsA2uiSurfaceSemantics,
  type A2uiCreateSurfaceMessageV1,
  type AgentopsA2uiSemanticOptionsV1,
  type AgentopsA2uiSurfaceV1,
} from "./a2ui.js";
import {
  AGENTOPS_VIEW_SPEC_MAX_BYTES,
  analyzeAgentopsViewSpecSemantics,
  type AgentopsViewSpecV1,
} from "./view-spec.js";

const CORE_GENERATED_VIEW_KIND = "com.agentops.core/generated_view";
...
function jsonLimitsForSchema(schemaName: string): Partial<GenerativeUiJsonValueLimits> {
  if (schemaName === "agentops-view-spec-v1") return { max_bytes: AGENTOPS_VIEW_SPEC_MAX_BYTES };
  if (schemaName === "agentops-a2ui-surface-v1") return { max_bytes: AGENTOPS_A2UI_MAX_BYTES };
...
function nodeSemanticErrors(
  ...
  if (node.view) {
    if (node.kind !== CORE_GENERATED_VIEW_KIND || node.kind_version !== LEGACY_VIEW_SPEC_KIND_VERSION) {
      errors.push({
        path: `${path}/view`,
        message: "legacy ViewSpec is supported only for com.agentops.core/generated_view@1",
        keyword: "legacyViewSpecVersion",
      });
    }
    const viewAnalysis = analyzeGenerativeUiJsonValue(node.view, {
      path: `${path}/view`,
      limits: { max_bytes: AGENTOPS_VIEW_SPEC_MAX_BYTES },
    });
    if (!viewAnalysis.valid) {
      errors.push({
        path: viewAnalysis.error?.path || `${path}/view`,
        message: viewAnalysis.error?.message || `view exceeds ${AGENTOPS_VIEW_SPEC_MAX_BYTES} bytes`,
        keyword: viewAnalysis.error?.keyword || "maxPayloadBytes",
      });
    } else {
      errors.push(...analyzeAgentopsViewSpecSemantics(node.view, {
  ...
  if (node.a2ui) {
    if (node.kind === CORE_GENERATED_VIEW_KIND && node.kind_version !== NATIVE_A2UI_KIND_VERSION) {
      errors.push({
        path: `${path}/a2ui`,
        message: "native A2UI presentation for com.agentops.core/generated_view requires kind version 2",
        keyword: "nativeA2uiVersion",
      });
    }
    const a2uiAnalysis = analyzeGenerativeUiJsonValue(node.a2ui, {
      path: `${path}/a2ui`,
      limits: { max_bytes: AGENTOPS_A2UI_MAX_BYTES },
    });
    if (!a2uiAnalysis.valid) {
      errors.push({
        path: a2uiAnalysis.error?.path || `${path}/a2ui`,
        message: a2uiAnalysis.error?.message || `a2ui exceeds ${AGENTOPS_A2UI_MAX_BYTES} bytes`,
        keyword: a2uiAnalysis.error?.keyword || "maxPayloadBytes",
      });
    } else {
      errors.push(...analyzeAgentopsA2uiSurfaceSemantics(node.a2ui, {
        action_ids: new Set((node.actions ?? []).map((action) => action.id)),
...
export function validateAgentopsViewSpec(
  value: unknown,
  options: { action_ids?: ReadonlySet<string> } = {},
): GenerativeUiValidationResult<AgentopsViewSpecV1> {
  const validation = validate<AgentopsViewSpecV1>("agentops-view-spec-v1", value);
  ...
  return withSemanticErrors(validation, analyzeAgentopsViewSpecSemantics(validation.value, options));
}

export function validateAgentopsA2uiSurface(
  value: unknown,
  options: Omit<AgentopsA2uiSemanticOptionsV1, "path"> = {},
): GenerativeUiValidationResult<AgentopsA2uiSurfaceV1> {
  const validation = validate<AgentopsA2uiSurfaceV1>("agentops-a2ui-surface-v1", value);
  if (!validation.value) return validation;
  const analysis = analyzeGenerativeUiJsonValue(validation.value, {
    limits: { max_bytes: AGENTOPS_A2UI_MAX_BYTES },
  });
  ...
  return withSemanticErrors(validation, analyzeAgentopsA2uiSurfaceSemantics(validation.value, options));
}

export function validateAgentopsA2uiCreateSurfaceMessage(
  value: unknown,
): GenerativeUiValidationResult<A2uiCreateSurfaceMessageV1> {
  return validate<A2uiCreateSurfaceMessageV1>(
    "agentops-a2ui-create-surface-message-v1",
    value,
  );
}
    ...
    if (operation.op === "patch") {
      ...
      if (operation.changes.view) {
        ...
        if (!viewAnalysis.valid) {
          ...
          errors.push(...analyzeAgentopsViewSpecSemantics(operation.changes.view, {
      ...
      if (operation.changes.a2ui) {
        ...
        if (!a2uiAnalysis.valid) {
          ...
          errors.push(...analyzeAgentopsA2uiSurfaceSemantics(operation.changes.a2ui, {
            ...(operation.changes.actions
              ? { action_ids: new Set(operation.changes.actions.map((action) => action.id)) }
